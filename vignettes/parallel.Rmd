...
**Note that these instructions apply to Mac/Linux systems**. On Mac/Linux, all loaded packages are made availalbe to all cores by default by "forking", meaning every core has access to *rmaverick* functions. In Windows you must instead use the *Parallel Socket Cluster* (PSOCK), which starts out with base packages only and any additional packages must be loaded through the `clusterEvalQ()` function.
