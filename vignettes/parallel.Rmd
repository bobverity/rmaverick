...
**Note that these instructions are only exactly correct for Mac/Linux systems**. On Mac/Linux, all loaded packages are made availalbe to all cores by default by "forking", meaning every core has access to *rmaverick* functions. In Windows you must instead use the *Parallel Socket Cluster* (PSOCK), which starts out with base packages only and any additional packages must be loaded through the `clusterEvalQ()` function. See 
...
We can then run the MCMC using the usual `run_mcmc()` function, this time passing in the cluster as an argument. This causes *rmaverick* to use a `clusterApplyLB()` call rather than an ordinary `lapply()` call when submitting different values of K. Note that output is supressed when running in parallel to avoid sending print commands to multiple cores, so you will not see the usual progress bars.
